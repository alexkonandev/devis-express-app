...
export const InteractiveQuote = ({
  ...
  return (
    <div className="w-[210mm] min-h-[297mm] bg-white shadow-2xl text-neutral-900 p-12 md:p-16 flex flex-col relative animate-in fade-in zoom-in-95 duration-300 select-none transition-all">
      ...
      <div className="flex justify-between items-start mb-12">
        <div>
          ...
          <div className="text-sm text-neutral-500 font-mono space-y-1">
            <p>
              N° <span className="text-black font-bold">{quote.quote.number}</span>
            </p>
      ...
      {/* Client - Editable */}
      <div className={`mb-16 group relative p-4 -mx-4 rounded-lg transition-colors border border-transparent ${!readOnly && "hover:bg-neutral-50 hover:border-neutral-200"}`}>
        {!readOnly && (
        ...
        {readOnly ? (
          <div className="space-y-1">
            <div className="text-xl font-bold">{quote.client.name || "Nom du client"}</div>
      ...
      <div className="mb-8">
        ...
        <div className="divide-y divide-neutral-100">
          {quote.items.map((item: LineItem, i: number) => (
            <div
              key={i}
              className={`group grid grid-cols-12 py-4 text-sm items-start -mx-2 px-2 rounded-md transition-colors relative ${!readOnly && "hover:bg-neutral-50"}`}
            >
              <div className="col-span-6 pr-4 space-y-1">
                {readOnly ? (
                  <>
                    <div className="font-bold text-neutral-800">{item.title}</div>
                    <div className="text-neutral-500 text-xs whitespace-pre-wrap">{item.subtitle}</div>
                  </>
                ) : (
                  <>
                    <input
                      value={item.title}
                      onChange={(e) => onUpdateLineItem(i, "title", e.target.value)}
                      placeholder="Titre de la prestation"
                      className="w-full font-bold text-neutral-800 bg-transparent focus:outline-none placeholder:text-neutral-300"
                    />
                    <TextareaAutosize
                      value={item.subtitle}
                      onChange={(e) => onUpdateLineItem(i, "subtitle", e.target.value)}
              ...
              <div className="col-span-2 text-center">
                {readOnly ? (
                  ...
                  <input
                    type="number"
                    value={item.quantity}
                    onChange={(e) => onUpdateLineItem(i, "quantity", parseFloat(e.target.value))}
              ...
              <div className="col-span-2 text-right">
                {readOnly ? (
                  <span className="text-neutral-600">{item.unitPriceEuros}</span>
                ) : (
                  <input
                    type="number"
                    value={item.unitPriceEuros}
                    onChange={(e) => onUpdateLineItem(i, "unitPriceEuros", parseFloat(e.target.value))}
      ...
      <div className="flex justify-end mt-auto pt-8">
        <div className="w-72 space-y-4">
          ...
          <div className="flex justify-between text-sm text-emerald-600 items-center group relative">
             <span className={`border-b border-dashed border-emerald-200 ${!readOnly && "cursor-help"}`}>Remise (€)</span>
             {readOnly ? (
               <span className="font-bold">{quote.financials.discountAmountEuros}</span>
             ) : (
               <input
                  type="number"
          ...
          <div className="flex justify-between text-sm text-neutral-500 pb-4 border-b border-neutral-200 items-center">
            <span className={`border-b border-dashed border-neutral-200 ${!readOnly && "cursor-help"}`}>TVA (%)</span>
            {readOnly ? (
      ...
      <div className={`mt-20 pt-8 border-t-2 border-neutral-100 text-[10px] text-neutral-400 text-center uppercase tracking-widest group relative`}>
        {!readOnly && (
