...
import { ServiceItem, ItemInput } from "@/lib/types"; // Import unifié
import {
  EditableTextCell,
  EditablePriceCell,
  CategorySelectCell,
...
import { PackageOpen, Pencil, Sparkles } from "lucide-react";
      ...
      <table className="min-w-[1100px] w-full border-collapse table-fixed text-left text-sm">
        <thead className="bg-zinc-50 sticky top-0 z-20 border-b border-zinc-200">
          <tr className="h-10">
            ...
            <th className="w-20 px-4 font-bold text-[11px] text-zinc-600 uppercase tracking-wide text-center select-none border-r border-zinc-200">
              TVA
            </th>
        ...
        <tbody className="bg-white divide-y divide-zinc-200">
          {items.length === 0 ? (
            ...
            items.map((item) => {
              // Détection d'item riche
              const isRich =
                !!item.pricing?.tiers ||
                (item.technicalScope?.included?.length || 0) > 0;
              ...
              return (
                ...
                >
                  {/* ID + SPARKLES */}
                  <td className="px-4 border-r border-zinc-100 font-mono text-[11px] font-medium text-zinc-500 bg-zinc-50/30">
                    <div className="flex items-center gap-2">
                      ...
                      {isRich && (
                        <Sparkles
                          className="w-3 h-3 text-amber-500 fill-amber-100 animate-in zoom-in"
                          title="Item Riche (Ontologie)"
                        />
                      )}
                    </div>
                  </td>
                  ...
                  <td className="border-r border-zinc-100 p-0 relative h-12 group/cell hover:bg-indigo-50/30 transition-colors cursor-text">
                    ...
                    <EditableTextCell
                      ...
                      className={cn(
                        "font-semibold text-sm px-4 h-full flex items-center",
                        isRich ? "text-indigo-900" : "text-zinc-900"
                      )}
                  ...
                  <td className="p-0 relative h-12 group/cell hover:bg-indigo-50/30 transition-colors cursor-text">
                    ...
                    <EditablePriceCell
                      ...
                      onUpdate={(val) =>
                        onUpdate(item.id, "unitPriceEuros", val)
                      }
                    />
                  </td>
                </tr>
